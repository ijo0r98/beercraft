...
from .models import DetailInfo, Comment ,Recomment
...
def detail(request, DetailInfo_id):
    beer_link = request.POST.get('beer_link')
    beer_alc = request.POST.get('beer_alc')
    beer_info = DetailInfo.objects.filter(id=DetailInfo_id)
    beer_comment = Comment.objects.filter(post=DetailInfo_id).order_by('-voter')
    context = {
        'beer_link':beer_link,
        'beer_alc':beer_alc,
        'beer_info':beer_info,
        'beer_comment':beer_comment,
        'beer_id':DetailInfo_id,
        }
    return render(request, 'beer/portfolio-details.html' ,context)

def review_create(request, DetailInfo_id):

    post = get_object_or_404(DetailInfo, pk=DetailInfo_id)
    comment = Comment(post=post, comment_text=request.POST.get('content'), created_at=timezone.now())
    comment.author = request.user
    comment.save()
    return redirect('detail',DetailInfo_id=DetailInfo_id)

def index(request):
    DetailInfo_list = DetailInfo.objects.all()
    context = {'DetailInfo_list': DetailInfo_list}
...
def comment_delete(request, comment_id):
    ...
    if request.user != comment.author:
        ...
        comment.delete()
    return redirect('detail', DetailInfo_id=comment.post.id)
...
def comment_create_answer(request, answer_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment.save()
            return redirect('detail', DetailInfo_id=comment.answer.post.id)
    else:
        form = RecommentForm()
    context = {'form': form}
    return render(request, 'beer/comment_form.html', context)
...
def comment_modify_answer(request, comment_id):
    comment = get_object_or_404(Recomment, pk=comment_id)
    if request.user != comment.author:
        messages.error(request, '댓글수정권한이 없습니다')
        return redirect('detail', DetailInfo_id=comment.answer.post.id)

    if request.method == "POST":
        form = RecommentForm(request.POST, instance=comment)
        if form.is_valid():
            comment = form.save(commit=False)
            comment.modify_date = timezone.now()
            comment.save()
...
def ver4(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'beer/ver4.html', {})
        ...
        return render(request, 'beer/ver4_result.html', {
            'beer_list':beer_list,
            'beer_result_first':beer_result_first,
...
def ver2(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(1,11):
            if request.POST.get('beer' + str(i), '') == '':
                break
            else:
        ...
        return render(request, 'beer/ver2.html', {'beer_list': beer_list, 'beer_str': beer_str})
